...
app.use(cookieParser());
app.use(cors(corsOptions));
app.options("*", cors(corsOptions)); // ✅ CORS Preflight support
...
// ✅ Contract auto-dissolution logic
const scheduleDissolutionCheck = () => {
  const runDissolutionCheck = async () => {
    try {
      const sevenDaysAgo = new Date(Date.now() - 7 * 24 * 60 * 60 * 1000);
      ...
      for (const contract of contracts) {
        if (!contract?.contractNumber || isNaN(new Date(contract.createdAt))) continue;
      ...
      console.error("Dissolution check error:", err.message);
    }
  };
  ...
  setTimeout(() => {
    runDissolutionCheck();
    setInterval(runDissolutionCheck, 24 * 60 * 60 * 1000);
  }, initialDelay);
};
